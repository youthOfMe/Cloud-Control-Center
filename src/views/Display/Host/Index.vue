...
import { reqPeriodData } from '@/api/timing'
import { getPowerScreenData } from '@/api/display/host'
import defaultHomeData from '../../config/default-home-data.json'
import { reqHostCount, reqHostIncrease, reqHostTop } from '@/api/display/host/index.js'

const powerData = ref(defaultHomeData)

// 发起网络请求
const getPowerScreen = () => {
    getPowerScreenData().then((res) => {
        powerData.value = res.data
        reqHostCount().then((response) => {
            console.log(response)
            powerData.value.chargingPile.data = response.data
        })
        reqHostIncrease().then((response) => {
            console.log(response)
            powerData.value.dataAnalysis.data = response.data
        })
        reqHostTop().then((response) => {
            powerData.value.chargingTop4.totalPercentage = response.totalPercentage
            powerData.value.chargingTop4.data = response.data
        })
    })
}
