<template>
    <div class="multi_exec">
        <el-card>
            <div>
                ...
                <div>
                    <a-tag
                        closable
                        @close="close_host(info_index)"
                        v-for="(info, info_index) in show_host_info"
                        :key="info.id"
                    >
            ...
            <div style="margin-top: 10px">
                <a-button @click="showModal" icon="plus" type="primary">从主机列表中选择</a-button>
                <a-button @click="showModal2" type="primary" style="margin-left: 15px">
                    从执行模板中选择
                </a-button>
            </div>
            ...
            <div style="margin-top: 15px">
                <a-button type="primary" icon="thunderbolt" @click="execute_cmd" class="fr">
                    开始执行
                </a-button>
                <div class="clear"></div>
            </div>
        ...
        <div>
            <a-modal
                v-model:visible="MultiExecVisible"
                title=""
                @ok="onMultiExecSubmit"
...
import { reqCategoryList, reqHostList } from '@/api/host'
import { cmdExec } from '@/api/task/executeTask'
import { reqTemplateList, reqTemplateCategory } from '@/api/task/manageTask'
...
export default {
    ...
    created() {
        this.get_host_category_list() / this.get_host_list()
    ...
    methods: {
        ...
        get_templates_list(category = null) {
            ...
            reqTemplateList(params)
                .then((response) => {
                    this.tem_data = response.results
                })
                .catch((error) => {
                    ElMessage({
                        message: '获取任务模板列表失败!请联系管理员！',
                    })
                })
        },
        get_templates_category_list() {
            reqTemplateCategory()
                .then((response) => {
                    this.tem_categorys = response.results
                })
                .catch((error) => {
                    ElMessage({
                        message: '获取任务模板类别列表失败!请联系管理员！',
                        type: 'error',
                    })
                })
        },
