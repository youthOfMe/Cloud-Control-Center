<template>
        ...
        <a-row>
            <a-col :span="20">
                <div class="add_host" style="margin: 15px">
                    <a-button @click="showProjectModal" type="primary">新建</a-button>
                </div>
            </a-col>
        </a-row>
        <div class="release">
    ...
    <!-- 添加项目 -->
    <a-modal
        v-model:visible="projectModelVisible"
        title="新建仓库"
        @ok="handleadProjectOk"
        ok-text="添加"
        cancel-text="取消"
    >
        <a-form
            ref="addProjectRuleForm"
            :model="project_form"
            :rules="add_project_rules"
            :label-col="labelCol"
            :wrapper-col="wrapperCol"
        >
            <a-form-item ref="name" label="仓库名称" name="name">
                <a-input v-model:value="project_form.name" />
            </a-form-item>
            <a-form-item ref="path" label="仓库路径" name="path">
                <a-input v-model:value="project_form.path" />
            </a-form-item>
            <a-form-item label="仓库描述" name="description">
                <a-textarea v-model:value="project_form.description" />
            </a-form-item>
            <a-form-item ref="default_branch" label="默认分支" name="default_branch">
                <a-select ref="select" v-model:value="project_form.default_branch">
                    <a-select-option value="main" key="main">main</a-select-option>
                    <a-select-option value="master" key="master">master</a-select-option>
                </a-select>
            </a-form-item>
            <a-form-item ref="visibility" label="可见度" name="visibility">
                <a-select ref="select" v-model:value="project_form.visibility">
                    <a-select-option
                        :value="key"
                        v-for="(visibility, key) in visibility_list"
                        :key="key"
                    >
                        {{ visibility }}
                    </a-select-option>
...
import { reqGitList, reqBranchList, reqCommitList, addGit } from '@/api/release/git'
...
export default {
    ...
    setup() {
        ...
        const get_branch_list = () => {
            reqBranchList(current_project_id.value)
                .then((response) => {
        ...
        const get_commit_list = () => {
            // 获取指定仓库的历史版本
            reqCommitList(current_project_id.value)
                .then((response) => {
                    commitList.value = response
                })
                .catch((error) => {
                    ElMessage({
                        message: '获取历史版本数据失败',
        ...
        const add_project_rules = reactive({
            // 添加发布应用的表单数据验证规则
            name: [
